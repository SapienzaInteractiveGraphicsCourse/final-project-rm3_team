	...
	constructor(params) {
		...
		if(!this.dead){
			...
			if(this.body.position.y<8) {
				...
				this.body.velocity.x *= Math.pow(0.99,time/10);
				if(this.body.velocity.y>0) this.body.velocity.y *= Math.pow(0.99,time/10);
				this.body.velocity.z *= Math.pow(0.99,time/10);
			}
